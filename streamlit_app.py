...
gebührensatzung = st.selectbox('Welche Gebührensatzung soll verwendet werden?', load_gebührensatzung(data), index=0)

# In welchem Studiengang wird absolviert?
st.write('## Erbrachte Leistungen')
st.write('### In welchem Studiengang wird absolviert? 👨🏻‍🎓')
abs_studiengang = st.selectbox('Worin will man absolvieren?', load_studiengänge(data, gebührensatzung), index=0)

st.write("Diese Daten liegen vor - nur zur Demo:")
st.write(next((eintrag for eintrag in data[gebührensatzung] if eintrag["Studiengang"] == abs_studiengang), None))
...
v2 = st.selectbox('Wurde vorher schon etwas am CAS erbracht, was angerechnet werden kann (wie Semester in anderen Studiengängen oder Zertifikate?)', ('Nein', 'Ja, anderer Studiengang', "Ja, Zertifikate"), index=0)
if v2 == 'Ja, anderer Studiengang':
    st.write('#### Anrechnung von CAS Semestern aus anderem Studiengang')
    v2_studiengang = st.selectbox('Welcher Studiengang wurde vorher absolviert?', load_studiengänge(data, gebührensatzung))
