...
with st.container(border=True):
    ...
    col_studiengang, col_semester = st.columns(2)
    with col_studiengang:
    ...
    # Initialwert
    geschätzte_gesamtgebühr: float = (
        float(studiengang_data.get("Studiengebühren", 0.0)) * 4
    )
...
with st.container(border=True):
    st.header("Anrechnungen")
    st.subheader("Anrechnung externer Leistungen")
    if fachbereich in {"Sozialwesen", "Gesundheit"}:
        ...
        if v1 == "Ja":
            ...
            studiengebuehren: float = float(
                studiengang_data.get("Studiengebühren", 0.0)
            )

            geschätzte_gesamtgebühr = db3 * 4 + (
                (((db1 + db2) * 4) / 90.0) * (90 - ects_extern)
            )
    ...
    st.subheader("Anrechnung von Zeit am CAS")
    v2: str = st.radio(
